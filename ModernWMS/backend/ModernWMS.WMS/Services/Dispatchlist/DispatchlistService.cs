...
namespace ModernWMS.WMS.Services
{
    ...
    public class DispatchlistService : BaseService<DispatchlistEntity>, IDispatchlistService
    {
        ...
        private readonly IInventoryWebhookOutbox _inventoryWebhookOutbox;
        ...
        public DispatchlistService(
          ...
          , IStringLocalizer<ModernWMS.Core.MultiLanguage> stringLocalizer
           ...
           , IInventoryWebhookOutbox inventoryWebhookOutbox
            )
        {
            this._dBContext = dBContext;
            this._stringLocalizer = stringLocalizer;
            this._functionHelper = functionHelper;
            this._inventoryWebhookOutbox = inventoryWebhookOutbox;
        }
        ...
        public async Task<(bool flag, string msg)> Delivery(List<DispatchlistDeliveryViewModel> viewModels, CurrentUser currentUser)
        {
            ...
            if (res > 0)
            {
                var skuIds = picks.Select(p => p.sku_id).Distinct().Select(id => (id, "")).ToList();
                var dispatchNo = entities.FirstOrDefault()?.dispatch_no ?? "";
                await _inventoryWebhookOutbox.EnqueueManyAsync(
